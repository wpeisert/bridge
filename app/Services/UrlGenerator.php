...
class UrlGenerator implements UrlGeneratorContract
{
    ...
    public function signedRoute($name, $parameters = [], $expiration = null, $absolute = true)
    {
        ...
        $route = $this->route($name, $parameters + [
                'signature' => hash_hmac('sha256', $this->route($name, $parameters, $absolute), $key),
            ], $absolute);

        Log::debug('signedRoute',
            [
                'route' => $route,
                'key' => call_user_func($this->keyResolver),
            ]
    ...
    public function hasCorrectSignature(Request $request, $absolute = true, array $ignoreQuery = [])
    {
        ...
        $eq = hash_equals($signature, (string)$request->query('signature', ''));

        // xxxxxxxxxxxxxxxxxxxxxxxxxxx
        Log::debug('hasCorrectSignature',
            [
                'original url' => $original,
                'key' => call_user_func($this->keyResolver),
                'signature' => $signature,
                'query signature' => (string)$request->query('signature', ''),
                'hash_equals' => ($eq ? 'TRUE' : 'FALSE'),
            ]
        );
